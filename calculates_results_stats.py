...
#          following keys:
#            n_images - number of images #CREATED LATER IN PROGRAM on line 138 #777
#            n_dogs_img - number of dog images #777, created line 79, incremenation transpires on 108
#            n_notdogs_img - number of NON-dog images #CREATED LATER IN PROGRAM on line 141 #777
#            n_match - number of matches between pet & classifier labels #777 , created line 80, incremenation on 90
#            n_correct_dogs - number of correctly classified dog images #777 , created line 81
#            n_correct_notdogs - number of correctly classified NON-dog images  #777 , created line 82
#            n_correct_breed - number of correctly classified dog breeds  #777 , created line 83
#            pct_match - percentage of correct matches  #777 , created line 151
#            pct_correct_dogs - percentage of correctly classified dogs #777 , created line 159
#            pct_correct_breed - percentage of correctly classified dog breeds #777 , created line 168
...
def calculates_results_stats(results_dic):
    ...
    # results_stats_dic['n_images'] = 0 #number of images, created later in program
    ...
    for key in results_dic:
        ...
        if results_dic[key][2] == 1 and results_dic[key][3] == 1:  #SLP Authored Logic
            results_stats_dic['n_correct_breed'] += 1
        
        # Pet Image Label is a Dog - counts number of dog images
        if results_dic[key][3] == 1:  #pet label at index 3 is a dog(1)
            results_stats_dic['n_dogs_img'] += 1
            
            # Classifier classifies image as Dog (& pet image is a dog)
            # counts number of correct dog classifications
            if results_dic[key][4] == 1:  #this says 'did the classifier actually classify it correctly?'
        ...
        else:  #meaning 'pet label is NOT a dog'
            # Classifier classifies image as NOT a Dog(& pet image isn't a dog)
            # counts number of correct NOT dog clasifications.
            if results_dic[key][4] == 0: #SLP Authored Logic #classifier label at index 4 is NOT a dog(0)
                results_stats_dic['n_correct_notdogs'] += 1
                
    # Calculates run statistics (counts & percentages) below that are calculated
    # using the counters from above.
    
    # calculates number of total images #777 AND CREATES n_images
    results_stats_dic['n_images'] = len(results_dic)

    # calculates number of not-a-dog images using - images & dog images counts #777 AND CREATES n_notdogs_img 
    results_stats_dic['n_notdogs_img'] = (results_stats_dic['n_images'] - 
                                      results_stats_dic['n_dogs_img']) #subtracts total number from Pet Label(1) IS DOG to find (total not dog)
    ...
    #    
    # Calculates % correct for matches #777 AND CREATES pct_match 
    ...
    #    
    # Calculates % correct dogs #777 AND CREATES pct_correct_dogs
    ...
    #           will need to be multiplied by 100.0 to provide the percentage.
    #    
    # Calculates % correct breed of dog #777 AND CREATES pct_correct_breed
    results_stats_dic['pct_correct_breed'] = (results_stats_dic['n_correct_breed'] / results_stats_dic['n_dogs_img']) * 100.0

    # Calculates % correct not-a-dog images #777 AND CREATES pct_correct_notdogs
    # Uses conditional statement for when no 'not a dog' images were submitted 
    # 777basically divides the number of correctly classified NON-dogs by the total number of 
    # NON-dog images themseleves to determine a percentage
